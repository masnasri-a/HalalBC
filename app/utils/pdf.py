...
def Lampiran(nama: str, ktp, no, jabatan, perusahaan, alamat, region):
    try:
        ...
        pdf.cell(200, 15, txt=" SURAT PERNYATAAN DAFTAR ALAMAT FASILITAS PRODUKSI", align='C', ln=1)
        pdf.set_font('helvetica', style="B")
        pdf.cell(200, 2, txt="  DAN BEBAS DARI BABI DAN TURUNANNYA", align='C', ln=1)

        pdf.set_font("Arial", size=12)
        pdf.cell(200, 60, "      Saya yang bertanda tangan dibawah ini: ", ln=1)
        ...
        pdf.cell(200, 5, '      Menyatakan bahwa alamat produksi untuk Perusahaan ' +
        ...
        pdf.cell(200, 25, "      "+region+", "+get_time_parse(), ln=1)
        pdf.cell(200, -10, "      "+jabatan, ln=1)
        pdf.cell(200, 70, "      (  "+nama+"  )")
        paths = "data"+str(nama.replace(' ','_'))+".pdf"
        pdf.output(paths)
        return paths
